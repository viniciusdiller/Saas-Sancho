...
import { getRoomTypeAvailability, updateAvailability } from '@/services/channex/ari';
...
export async function GET(request: Request) {
  ...
  const data = await getRoomTypeAvailability({
    propertyId,
...
export async function POST(request: Request) {
  ...
  const data = await updateAvailability(propertyId, body);
  return NextResponse.json(data);
}
